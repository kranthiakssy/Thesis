...
from td3_module import CriticNetwork, ActorNetwork, ReplayBuffer

# Agent Creation and Using torch for cuda support
class Agent():
    def __init__(self, alpha, beta, input_dims, tau, env,
            gamma=0.99, update_actor_interval=2, warmup=1000,
            n_actions=2, max_size=1000000, layer1_size=400,
            layer2_size=300, batch_size=100, noise=0.2, action_bound=1):
        self.gamma = gamma
        self.tau = tau
        ...
        self.actor = ActorNetwork(alpha, input_dims, layer1_size,
                        ...
                        action_bound=action_bound, name='actor')
        ...
        self.target_actor = ActorNetwork(alpha, input_dims, layer1_size,
                    layer2_size, n_actions=n_actions, 
                    action_bound=action_bound, name='target_actor')
    ...
    def choose_action(self, observation):
        if self.time_step < self.warmup:
            mu = T.tensor(np.random.normal(scale=0.001, 
                                            size=(self.n_actions,)))
        else:
            state = T.tensor(observation, dtype=T.float).to(self.actor.device)
            mu = self.actor.forward(state).to(self.actor.device)
        mu_prime = mu + T.tensor(np.random.normal(scale=0.001),
